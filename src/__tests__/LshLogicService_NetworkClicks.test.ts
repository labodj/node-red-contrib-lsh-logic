...
describe("LshLogicService - Network Click Logic", () => {
  ...
  it("should clear only the click transactions that depend on the rebooted device", () => {
    const systemConfig: SystemConfig = {
      devices: [
        {
          name: "device-sender",
        ...
        {
          name: "device-sender-2",
          longClickButtons: [
            {
              id: 1,
              actors: [{ name: "actor2", allActuators: true, actuators: [] }],
              otherActors: [],
            },
          ],
        },
    ...
    const { setDeviceOnline, sendMisc } = createClickHarness(systemConfig);
    setDeviceOnline("actor1");
    setDeviceOnline("actor2");
    setDeviceOnline("device-sender");
    setDeviceOnline("device-sender-2");

    startClick(sendMisc, "device-sender", ClickType.Long, 1, 7);
    startClick(sendMisc, "device-sender-2", ClickType.Long, 1, 8);

    const bootResult = sendMisc("actor1", { p: LshProtocol.BOOT_NOTIFICATION });
    const clearedConfirm = confirmClick(sendMisc, "device-sender", ClickType.Long, 1, 7);
    const survivingConfirm = confirmClick(sendMisc, "device-sender-2", ClickType.Long, 1, 8);

    expect(bootResult.logs).toContain(
      "Cleared 1 pending click transaction(s) because a device reboot invalidated in-flight assumptions.",
    );
    expect(clearedConfirm.warnings).toContain(
      "Received confirmation for an expired or unknown click: device-sender.1.1.7.",
    );
    expect(survivingConfirm.logs).toContain(
      "Click confirmed for device-sender-2.1.1.8. Executing logic.",
    );
  });
});
